	...
	fetchLocaleData: function( url, localeName, outputPath, cb, attempt ) {
		...
		http.get( url + "/" + localeName, function (res) {
			...
			res.on('end', function() {
				...
				data.territories = res.localeDisplayNames.territories;
				data.languages = res.localeDisplayNames.languages;
				...
				data.numbers.currencyFormats = res.numbers[ "currencyFormats-numberSystem-" + defaultNumberingSystem ];
				data.numbers.decimalFormats = res.numbers[ "decimalFormats-numberSystem-" + defaultNumberingSystem ];
				data.numbers.percentFormats = res.numbers[ "percentFormats-numberSystem-" + defaultNumberingSystem ];

				var result = fs.writeFileSync( outputPath + "/" + localeName, JSON.stringify( data ), 'utf8' );

				console.log( "Fetched:" + localeName, result );
